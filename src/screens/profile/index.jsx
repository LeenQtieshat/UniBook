...
const App = () => {
  ...
  useEffect(() => {
    (async () => {
      const { reservedBooks, likedBooks, comments } = await getUser(
        JSON.parse(localStorage.getItem("userData")).email
      );
      const books = await getBooks();
      if (reservedBooks) {
        const reserved = books.filter((book) =>
        ...
        <div className="container">
          ...
          >
            <Col>
              <div
                className="content"
                style={{ color: "rgb(225 217 171 / 81%)" }}
              >
                <Title
                  level={1}
                  className="heading"
                ...
                <Text className="subtext">
                  <Segmented
                    ...
                    options={["Reserved Books", "User Likes", "User Comments"]}
                    style={{ background: "rgb(225 217 171 / 81%)" }}
        ...
        >
          <List
            ...
            renderItem={(item, index) => (
              <List.Item style={{ fontSize: "18px", display: "flex", textAlign : isArabic(item) ? 'right' : 'left', direction :  isArabic(item) ? 'rtl' : 'ltr'}}>
                <div> {item} </div>
              </List.Item>
            )}
          />
