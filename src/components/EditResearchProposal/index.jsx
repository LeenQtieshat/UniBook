...
import { getResearch, updateResearchDetails } from '../../firebase/researches';
...
const EditResearchProposal = () => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [showSuccess, setSuccess] = useState(false);
  const navigator = useNavigate();
  const { id } = useParams(); // Assuming you pass the research ID via the URL

  useEffect(() => {
    const fetchResearch = async () => {
      const researchData = await getResearch(id);
      console.log("🚀 ~ fetchResearch ~ researchData:", researchData)
      form.setFieldsValue(researchData);
    };
  ...
  const onFinish = async (values) => {
    setSuccess(false);
    setLoading(true);
    const {id:doctorId} = JSON.parse(localStorage.getItem("userData"))
    await updateResearchDetails(id, {...values,doctorId});
    setLoading(false);
    setSuccess(true);
  };
      ...
      {showSuccess &&
        ((placement = "topLeft") => {
          notification.open({
            ...
            description: 'Your research has been successfully updated! Thank you for your contribution.',
            onClick: () => {},
          });
        })()}
      <Layout className="layout">
        <Content style={{ padding: '0 50px', background: 'rgb(225, 217, 171)', background: 'linear-gradient(90deg, rgba(225, 217, 171, 1) 0%,rgba(139, 125, 73, 1) 35%)' }}>
          <Breadcrumb style={{ margin: '16px 0' }}>
            ...
            <Breadcrumb.Item>Edit Research Proposal</Breadcrumb.Item>
          </Breadcrumb>
...
export default EditResearchProposal;
